<template>
  <div class="container">
    <div class="row">
      <div class="col-sm-10">
        <h1>Горнолыжный курорт "Лаура"</h1>
  ...
  <div>
    ...
    <div v-for="(comment, index) in comments" :key = "index" class="comments-box">
        ...
        <p v-if="admin!='user'"
        @click="removeComment(comment.email, comment.text)"
        class="delete">Удалить</p>
    </div>
</div>
  </div>
</template>
...
export default {
  ...
  methods: {
    getTraces() {
      const path = 'http://localhost:5005/api/v1/courort/laura';
      axios.get(path)
        .then((res) => {
    ...
    getComments() {
      this.$http.get('//localhost:5005/api/v1/comment/'+ "1")
            .then(response => {
    ...
    removeComment: function(e, t){
            this.$http.delete('//localhost:5005/api/v1/comment/'+t+'/'+e+'/'+1)
            .then(response => {
              alert('Success')
